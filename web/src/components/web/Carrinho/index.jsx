...
export default function CarrinhoCard({ item: { produto, qtd }, removeItem, ListProducts}) {
    const [qtdProduto, setQtdProduto] = useState(qtd)
    const navigate = useNavigate()

    function remove(){
        removeItem(produto.id_produtos)
    }

    const showImg = (imagem) => {
        return API_URL + '/' + imagem
    }

    function CalcSubTotal() {
        const subtotal = qtdProduto * produto.vl_valor
        return subtotal
    }

    function ChangeQtd(novaQtd) {
        setQtdProduto(novaQtd)

        let carrinho = Storage('carrinho')
        let itemStorage = carrinho.find(item => item.id == produto.id_produtos)
        ...
        <div className="carrinho-container">
            ...
            <div className='content-product'>
                ...
                <p>R$ {CalcSubTotal()}</p>
                    ...
                    <select onChange={e => ChangeQtd(e.target.value)} value={qtdProduto}>
                        <option>1</option>
                        <option>2</option>
                        <option>3</option>
                        <option>4</option>
